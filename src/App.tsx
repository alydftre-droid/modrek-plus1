...
// استعادة منطق الأقسام الأصلي بدقة
...
const Dashboard = () => {
  ...
  if (isLoading) return <div className="min-h-screen flex items-center justify-center bg-background"><Loader2 className="animate-spin text-primary h-12 w-12" /></div>;
      ...
      <header className="sticky top-0 z-50 w-full border-b border-border/50 bg-background/80 backdrop-blur-xl">
        <div className="container flex h-14 lg:h-16 items-center justify-between px-3 lg:px-4 max-w-full">
          ...
          <div className="flex items-center gap-1 lg:gap-2">
            ...
            {/* التعديل المطلوب: تفعيل زر الإعدادات */}
            <Button 
              variant="ghost" 
              size="icon" 
            ...
            <Button variant="ghost" size="icon" onClick={handleSignOut} className="hover:text-destructive h-8 w-8 lg:h-10 lg:w-10"><LogOut className="h-4 w-4 lg:h-5 lg:w-5" /></Button>
          </div>
        </div>
      </header>
