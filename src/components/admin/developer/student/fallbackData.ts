...
export async function fetchStudentTeachersFallback(studentId: string) {
  ...
  groups.forEach((group: AnyRow) => {
    ...
    const old = byTeacher.get(teacherId) ?? { teacher_id: teacherId, teacher_name: teacherMap.get(teacherId)?.full_name ?? null, avatar_url: teacherMap.get(teacherId)?.avatar_url ?? null, courses_count: 0, total_paid: 0, last_interaction: null };
    old.courses_count += 1;
    old.total_paid += paid;
