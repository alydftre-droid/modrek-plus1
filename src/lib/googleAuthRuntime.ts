...
export type GoogleAuthRuntimeHealth = {
  ...
  browserPluginAvailable: boolean;
  appPluginAvailable: boolean;
  canAttemptNative: boolean;
  canUseBrowserFallback: boolean;
  errors: string[];
  warnings: string[];
};
...
export async function getGoogleAuthRuntimeHealth(): Promise<GoogleAuthRuntimeHealth> {
  ...
  try {
    ...
    browserPluginAvailable = Capacitor.isPluginAvailable("Browser");
  ...
  if (isNativeRuntime && !browserPluginAvailable) errors.push("BROWSER_PLUGIN_NOT_AVAILABLE_FOR_OAUTH_FALLBACK");
  if (isNativeRuntime && !appPluginAvailable) errors.push("APP_PLUGIN_NOT_AVAILABLE_FOR_DEEP_LINK_CALLBACK");

  warnings.push("SHA_FINGERPRINTS_AND_GOOGLE_OAUTH_CLIENTS_ARE_ENFORCED_BY_CI_CONFIG_TEST");

  const canAttemptNative = isNativeRuntime
    && webClientIdPresent
    && webClientIdLooksValid
    && socialLoginPluginAvailable;
  const canUseBrowserFallback = isNativeRuntime && browserPluginAvailable && appPluginAvailable;
...
export function validateGoogleIdTokenForConfiguredClient(idToken: string) {
  try {
    const [, payload] = idToken.split(".");
    if (!payload) return { ok: false, error: "GOOGLE_ID_TOKEN_PAYLOAD_MISSING" };

    const normalizedPayload = payload.replace(/-/g, "+").replace(/_/g, "/");
    const parsed = JSON.parse(window.atob(normalizedPayload));
