...
export default function TeacherWalletPage() {
  ...
  if (view === "archive-detail" && selectedArchiveId) {
    ...
    const stats = snap?.stats || {
      ...
      commission_rate: Number(archive.commission_rate) || 0,
      subscribers: Number(archive.total_subscribers) || 0,
      groups: Number(archive.total_groups) || 0,
    };
    const grades = Array.isArray(snap?.grades) ? snap.grades : [];
    const groupsList = Array.isArray(snap?.groups) ? snap.groups : (archive.breakdown || []) as any[];
    const snapTxns = Array.isArray(snap?.transactions) ? snap.transactions : [];
    const snapWithdrawals = Array.isArray(snap?.withdrawals) ? snap.withdrawals : [];
    const paymentMethodsSnap = Array.isArray(snap?.payment_methods) ? snap.payment_methods : [];

    const archiveTransactions = snapTxns.length > 0 ? snapTxns : walletTransactions.filter((tx: any) => {
    ...
    const commissionPct = Math.round(Number(stats.commission_rate || archive.commission_rate || 0) * 100);
    const platformCut = Number(stats.platform_cut ?? (Number(stats.total_gross || 0) - Number(stats.total_earned || 0))) || 0;
      ...
      <TeacherSidebarLayout title="سجل شهر" teacherName={teacherName}>
        <div className="p-4 max-w-3xl mx-auto space-y-4">
          ...
          <div className="rounded-2xl border border-amber-200 bg-amber-50 px-3 py-2 text-[11px] text-amber-800 font-bold flex items-center gap-2">
            ...
            نسخة أرشيفية مثبتة لشهر {monthLabel(archive.period_label)} — لحظة الإقفال {snap?.captured_at ? new Date(snap.captured_at).toLocaleString("ar-EG", { timeZone: "Africa/Cairo" }) : new Date(archive.archived_at).toLocaleString("ar-EG", { timeZone: "Africa/Cairo" })}
          </div>

          <ArchiveHero
            label={monthLabel(archive.period_label)}
            earned={Number(stats.total_earned || archive.total_earned)}
            subs={Number(stats.subscribers || archive.total_subscribers)}
            groups={Number(stats.groups || archive.total_groups)}
          />

          {/* Wallet balances at closing */}
          {snap && (
            <SectionCard icon={<Wallet className="h-4 w-4" />} title="أرصدة المحفظة لحظة الإقفال">
              <div className="grid grid-cols-3 gap-2">
                <div className="rounded-2xl bg-emerald-50 border border-emerald-200 p-3 text-center">
                  <p className="text-[10px] text-emerald-700 font-bold">المتاح</p>
                  <p className="text-sm font-black text-emerald-800 mt-1">{fmtMoney(Number(walletBefore.balance || 0))} ج</p>
                </div>
                <div className="rounded-2xl bg-amber-50 border border-amber-200 p-3 text-center">
                  <p className="text-[10px] text-amber-700 font-bold">المجمّد</p>
                  <p className="text-sm font-black text-amber-800 mt-1">{fmtMoney(Number(walletBefore.frozen_balance || 0))} ج</p>
                </div>
                <div className="rounded-2xl bg-violet-50 border border-violet-200 p-3 text-center">
                  <p className="text-[10px] text-violet-700 font-bold">إجمالي الأرباح</p>
                  <p className="text-sm font-black text-violet-800 mt-1">{fmtMoney(Number(walletBefore.total_earned || 0))} ج</p>
                </div>
              </div>
            </SectionCard>
          )}
          ...
          <SectionCard icon={<BarChart3 className="h-4 w-4" />} title="إحصائيات الشهر">
            <div className="grid grid-cols-2 gap-2">
              ...
              <StatTile label="نصيب المنصة" value={`${fmtMoney(platformCut)} ج`} tone="rose" />
              <StatTile label="نسبة العمولة" value={`${commissionPct}%`} tone="violet" />
              <StatTile label="عدد الطلاب" value={String(Number(stats.subscribers || 0))} tone="amber" />
              <StatTile label="عدد المجموعات" value={String(Number(stats.groups || 0))} tone="indigo" />
              <StatTile label="عدد المواد" value={String(Number(stats.subjects || 0))} tone="teal" />
              <StatTile label="عدد الاشتراكات" value={String(Number(stats.subscriptions || stats.subscribers || 0))} tone="fuchsia" />
            </div>
          </SectionCard>

          {/* Per-grade breakdown */}
          {grades.length > 0 && (
            <SectionCard icon={<Users className="h-4 w-4" />} title="التوزيع حسب الصف الدراسي">
              <div className="space-y-2">
                {grades.map((gr: any, i: number) => (
                  <div key={i} className="p-3 rounded-2xl bg-muted/40 border border-border/50 flex items-center justify-between">
                    <div className="min-w-0">
                      <p className="text-sm font-bold">{formatGrade(gr.grade)} {formatStage(gr.stage)}{gr.category ? ` • ${gr.category}` : ""}</p>
                      <p className="text-[11px] text-muted-foreground">{gr.subscriberCount || 0} طالب • {gr.groupCount || 0} مجموعة • {gr.subjectCount || 0} مادة</p>
                    </div>
                    <p className="text-sm font-black text-emerald-700 shrink-0">{fmtMoney(Number(gr.totalEarned || 0))} ج</p>
                  </div>
                ))}
              </div>
            </SectionCard>
              ...
              <div className="space-y-2">
                {groupsList.map((g: any, j: number) => (
                  <div key={j} className="p-3 rounded-2xl bg-muted/40 border border-border/50">
                    ...
                    <CalcRow students={g.students} price={Number(g.price)} pct={Math.round(Number(g.commission_rate ?? archive.commission_rate) * 100)} net={Number(g.net)} />
              ...
              <div className="grid grid-cols-1 gap-2">
                {paymentMethodsSnap.map((pm: any) => (
                  <div key={pm.id} className="p-3 rounded-2xl bg-muted/40 border border-border/50 flex items-center justify-between">
                    <p className="text-sm font-bold">{pm.method_type}</p>
