...
export default function TeacherAssistantPage() {
  ...
  const firstName = useMemo(() => {
    ...
    return fullName.split(" ")[0] || "يا أستاذ";
  }, [user?.user_metadata?.full_name]);
  const draftKey = `teacher-support-draft-${user?.id || "guest"}`;
  ...
  useEffect(() => {
    if (!user?.id) return;
    setChatHistory(loadChatHistory<UiMessage>(`teacher-support-history-${user.id}`));
  }, [user?.id]);
  ...
  const saveCurrentChat = useCallback(() => {
    ...
    saveChatHistory(`teacher-support-history-${user.id}`, updated);
  ...
  const confirmEscalation = async () => {
    ...
    const { data: profile } = await supabase.from("profiles").select("full_name, teacher_code").eq("id", user.id).maybeSingle();
  ...
  const rejectEscalation = () => {
    setMessages(prev => prev.filter(m => m.role !== "escalate-confirm"));
    appendMessage({ id: `reject-${Date.now()}`, role: "assistant", content: "تمام! أنا هنا لمساعدتك. اسألني أي شيء آخر 😊", createdAt: new Date().toISOString() });
  };

  const streamAssistantReply = useCallback(
    async (payloadMessages: Array<{ role: string; content: unknown }>) => {
      if (!user) return;
      setLoading(true);
      try {
        ...
        appendMessage({ id: `assistant-${Date.now()}`, role: "assistant", content: cleaned || "تعذر الرد الآن، حاول مرة أخرى.", createdAt: new Date().toISOString() });
      } catch (error: any) {
        console.error(error);
  ...
  const uploadAttachment = useCallback(
    async (file: File, type: "image" | "audio") => {
      ...
      try {
        ...
        if (type === "image") {
          await streamAssistantReply(buildConversationPayload({ text, imageUrl: signedUrl }));
          return;
        }
        appendMessage({ id: `aa-${Date.now()}`, role: "assistant", content: "استلمت التسجيل 🎙️ أرسل وصفًا إضافيًا أو صورة وسأكمل معك.", createdAt: new Date().toISOString() });
      } catch (e: any) {
        console.error(e);
        toast.error(e?.message || "فشل رفع المرفق");
      } finally {
        setUploading(false);
      }
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="flex items-center justify-between h-14 px-4 border-b border-border bg-card shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-bold">{escalated ? "موظف الدعم" : "مساعد المعلم"}</p>
            ...
            <div className="flex flex-col items-center justify-center h-full py-12">
              ...
              <div className="flex flex-wrap gap-2 justify-center max-w-sm">
                {quickSuggestions.map((s, i) => (
                  <button key={i} onClick={() => setInput(s)} className="text-xs px-4 py-2 rounded-full bg-gradient-to-r from-blue-50 to-purple-50 text-blue-700 hover:from-blue-100 hover:to-purple-100 transition-colors font-medium border border-blue-200/50">
                    {s}
                  </button>
                ))}
              </div>
