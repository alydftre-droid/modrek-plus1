...
BEGIN
  ...
  IF w IN ('salah','salat','sala','prayer','pray','صلاه','صلوات','مصلي','يصلي') THEN RETURN 'صلاه'; END IF;
  ...
  IF w IN ('niyyah','niya','intention','intent','نيه','نوي','ينوي') THEN RETURN 'نيه'; END IF;
  ...
  IF w IN ('arafah','arafa','عرفه') THEN RETURN 'عرفه'; END IF;
  IF w IN ('zakat','zakah','زكاه') THEN RETURN 'زكاه'; END IF;
...
BEGIN
  ...
  IF public.is_exam_non_answer(_answer) THEN
    RETURN CASE WHEN COALESCE(trim(_answer), '') = '' THEN 'لم يجب الطالب على هذا السؤال.' ELSE 'لم يقدم الطالب إجابة قابلة للتصحيح لهذا السؤال.' END;
  END IF;
...
SELECT
  'repair.text_grading_transliteration_and_safe_feedback',
  ua.student_id,
  ua.exam_id,
  ua.id,
  jsonb_build_object(
    ...
    'ai_feedback_source', 'local_per_row_safe_feedback'
  )
FROM audit
...
REVOKE ALL ON FUNCTION public.exam_text_feedback(text, text, numeric, numeric) FROM PUBLIC, anon;
