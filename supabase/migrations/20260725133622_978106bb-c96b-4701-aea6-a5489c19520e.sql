...
GRANT EXECUTE ON FUNCTION public.exam_broadcast_group_ids(uuid) TO authenticated, anon, service_role;

-- Update the catalog RPC to use the broader broadcast-group set
...
-- Update RLS policy to use the broadcast helper
    ...
    OR (
      ...
      AND EXISTS (
        ...
        WHERE sgp.student_id = auth.uid()
          AND sgp.group_id IN (SELECT public.exam_broadcast_group_ids(exams.group_id))
      )
    )
  )
