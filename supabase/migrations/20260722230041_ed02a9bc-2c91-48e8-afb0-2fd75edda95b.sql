...
BEGIN
  ...
  -- Highest priority: if this month's configured day/time is due now and has not run,
  -- run it even when an old buggy save pushed withdrawal_next_release_at_cairo to next month.
  ...
  IF v_cairo_ts < v_due_ts THEN
    v_payload := jsonb_build_object(
      ...
      'current_month_due_cairo', to_char(v_current_due_ts, 'YYYY-MM-DD HH24:MI:SS'),
