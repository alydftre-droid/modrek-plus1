...
DECLARE
  ...
  _expected_track_code text;
  _subject_category text;
BEGIN
  IF NEW.subject_id IS NULL THEN
    RAISE EXCEPTION USING
      MESSAGE = 'library_debug.missing_subject_id',
      DETAIL = jsonb_build_object(
        'file', 'database:function:validate_library_book_scope_before_write',
        'function', 'validate_library_book_scope_before_write',
        'component', 'LibraryUploadPage',
        'hook', 'React useState/useMemo → callAdmin',
        'api', 'library-admin?action=create/update',
        'table', 'library_books',
        'column', 'subject_id',
        'sent_value', NEW.subject_id,
        'expected_value', 'public.subjects.id موجود وفعّال',
        'correct_value', null,
        'sql_query', 'SELECT id,name,category,stage,grade,section,is_active FROM public.subjects WHERE id = $1',
        'query_result', 'not_executed_missing_value',
        'failure_reason', 'subject_id_required',
        'source_table', 'public.subjects',
        'layer', 'database',
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION USING
      ...
      DETAIL = jsonb_build_object(
        ...
        'correct_value', null,
        'sql_query', 'SELECT id,name,category,stage,grade,section,is_active FROM public.subjects WHERE id = $1',
        'query_result', 'no_rows',
        'failure_reason', 'subject_id_not_found_in_public_subjects',
        'source_table', 'public.subjects',
        'forbidden_source_table', 'public.library_subjects',
        'insert_sql', 'INSERT INTO public.library_books (title, education_type, stage_id, grade_id, section_id, track_id, subject_id, ...) VALUES (...)',
        'layer', 'database',
  ...
  IF _subject.is_active IS NOT TRUE THEN
    RAISE EXCEPTION USING
      ...
      DETAIL = jsonb_build_object(
        ...
        'column', 'subject_id',
        'sent_value', NEW.subject_id,
        'expected_value', 'public.subjects.id فعّال',
        'correct_value', _subject.id,
        'query_result', row_to_json(_subject),
  ...
  IF NEW.stage_id IS NULL THEN
    RAISE EXCEPTION USING
      MESSAGE = 'library_debug.missing_stage_id',
  ...
  IF NEW.grade_id IS NULL THEN
    RAISE EXCEPTION USING
      MESSAGE = 'library_debug.missing_grade_id',
  ...
  IF NEW.section_id IS NULL THEN
    RAISE EXCEPTION USING
      MESSAGE = 'library_debug.missing_section_id',
  ...
  IF NEW.track_id IS NOT NULL THEN
    ...
    IF NOT FOUND OR _track.is_active IS NOT TRUE THEN
      RAISE EXCEPTION USING
        ...
        DETAIL = jsonb_build_object('file','database:function:validate_library_book_scope_before_write','function','validate_library_book_scope_before_write','component','LibraryUploadPage','hook','React useState/useMemo → callAdmin','api','library-admin?action=create/update','table','library_books','column','track_id','sent_value',NEW.track_id,'expected_value','public.library_tracks.id موجود وفعّال','sql_query','SELECT id,code,name_ar,is_active FROM public.library_tracks WHERE id = $1','query_result','no_rows_or_inactive','failure_reason','track_not_found_or_inactive','layer','database','error_type','validation_error')::text;
    END IF;
  END IF;
  ...
  _expected_track_code := CASE
    WHEN _track.code IN ('scientific', 'sci_science', 'sci_math') THEN 'scientific'
    WHEN _track.code = 'literary' THEN 'literary'
    ELSE _track.code
  END;
  ...
  IF _subject_stage_code IS DISTINCT FROM _stage.code THEN
    RAISE EXCEPTION USING
      MESSAGE = 'library_debug.invalid_subject_for_stage',
  ...
  IF _subject_grade_code IS DISTINCT FROM _grade.code THEN
    RAISE EXCEPTION USING
      MESSAGE = 'library_debug.invalid_subject_for_grade',
  ...
  IF _section.code = 'general' AND _subject_category IN ('sharia', 'religious') THEN
    RAISE EXCEPTION USING
      MESSAGE = 'library_debug.invalid_general_subject_category',
  ...
  IF NEW.track_id IS NOT NULL AND coalesce(_subject_track_code, '') <> '' AND _subject_track_code IS DISTINCT FROM _expected_track_code THEN
    RAISE EXCEPTION USING
      MESSAGE = 'library_debug.invalid_subject_for_track',
      DETAIL = jsonb_build_object('file','database:function:validate_library_book_scope_before_write','function','validate_library_book_scope_before_write','component','LibraryUploadPage','hook','React useState/useMemo → callAdmin','api','library-admin?action=create/update','table','library_books','column','subject_id','sent_value',NEW.subject_id,'expected_value','مادة تابعة للشعبة المحددة','correct_value',_subject.id,'selected_track_code',_track.code,'expected_source_track_code',_expected_track_code,'subject_track_code',_subject_track_code,'query_result',row_to_json(_subject),'failure_reason','subject_track_mismatch','source_table','public.subjects','layer','database','error_type','relationship_error')::text;
  END IF;

  IF NEW.track_id IS NOT NULL AND _track.code = 'sci_science' AND coalesce(_subject.name, '') LIKE '%رياضيات%' THEN
    RAISE EXCEPTION USING
      MESSAGE = 'library_debug.invalid_subject_for_sci_science_track',
      DETAIL = jsonb_build_object('file','database:function:validate_library_book_scope_before_write','function','validate_library_book_scope_before_write','component','LibraryUploadPage','hook','React useState/useMemo → callAdmin','api','library-admin?action=create/update','table','library_books','column','subject_id','sent_value',NEW.subject_id,'expected_value','مادة علمي علوم وليست رياضيات','correct_value',_subject.id,'subject_name',_subject.name,'selected_track_code',_track.code,'failure_reason','sci_science_track_cannot_use_math_subject','source_table','public.subjects','layer','database','error_type','relationship_error')::text;
  END IF;

  IF NEW.track_id IS NOT NULL AND _track.code = 'sci_math' AND (coalesce(_subject.name, '') LIKE '%أحياء%' OR coalesce(_subject.name, '') LIKE '%احياء%') THEN
    RAISE EXCEPTION USING
      MESSAGE = 'library_debug.invalid_subject_for_sci_math_track',
      DETAIL = jsonb_build_object('file','database:function:validate_library_book_scope_before_write','function','validate_library_book_scope_before_write','component','LibraryUploadPage','hook','React useState/useMemo → callAdmin','api','library-admin?action=create/update','table','library_books','column','subject_id','sent_value',NEW.subject_id,'expected_value','مادة علمي رياضة وليست أحياء','correct_value',_subject.id,'subject_name',_subject.name,'selected_track_code',_track.code,'failure_reason','sci_math_track_cannot_use_biology_subject','source_table','public.subjects','layer','database','error_type','relationship_error')::text;
  END IF;
