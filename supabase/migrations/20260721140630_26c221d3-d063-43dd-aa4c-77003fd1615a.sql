...
BEGIN
  IF COALESCE(_max_score, 0) <= 0 OR public.is_exam_non_answer(_answer_text) THEN
  ...
  IF v_exam_id IS NULL THEN
    RETURN jsonb_build_object(
      'model_answer', COALESCE(_current_model, ''),
      'source', 'current_model_answer',
      'current_score', public.smart_exam_text_score(_answer_text, COALESCE(_current_model, ''), _max_score),
      'previous_score', 0,
      'next_score', 0,
      'aligned', false
    );
  ...
  v_current_score := public.smart_exam_text_score(_answer_text, COALESCE(_current_model, ''), _max_score);
  v_prev_score := public.smart_exam_text_score(_answer_text, COALESCE(v_prev_model, ''), _max_score);
  v_next_score := public.smart_exam_text_score(_answer_text, COALESCE(v_next_model, ''), _max_score);
  ...
  IF COALESCE(trim(v_best_model), '') <> ''
     AND v_best_score >= COALESCE(_max_score, 0) * 0.60
     AND v_current_score <= COALESCE(_max_score, 0) * 0.40
     AND v_best_score >= v_current_score + (COALESCE(_max_score, 0) * 0.30) THEN
    RETURN jsonb_build_object(
...
CREATE OR REPLACE FUNCTION public.exam_text_feedback(
  _answer text,
  _model text,
  _score numeric,
  _max_score numeric,
  _alignment_source text DEFAULT 'current_model_answer'
)
RETURNS text
LANGUAGE plpgsql
IMMUTABLE
...
BEGIN
  ...
  IF public.is_exam_non_answer(_answer) THEN
    IF COALESCE(trim(_answer), '') = '' THEN
      RETURN 'لم يجب الطالب على هذا السؤال.';
    END IF;
    RETURN 'لم يقدم الطالب إجابة قابلة للتصحيح لهذا السؤال.';
  END IF;
  IF COALESCE(trim(_model), '') = '' THEN
    RETURN 'لا توجد إجابة نموذجية محفوظة لهذا السؤال؛ يحتاج مراجعة المعلم.';
  END IF;
  IF COALESCE(_alignment_source, 'current_model_answer') <> 'current_model_answer' THEN
    IF COALESCE(_score, 0) >= COALESCE(_max_score, 0) THEN
      RETURN 'إجابة صحيحة بالمعنى بعد إصلاح محاذاة الإجابة النموذجية لهذا السؤال.';
    END IF;
    IF COALESCE(_score, 0) > 0 THEN
      RETURN 'إجابة جزئية بعد إصلاح محاذاة الإجابة النموذجية لهذا السؤال.';
    END IF;
  END IF;
  IF COALESCE(_score, 0) >= COALESCE(_max_score, 0) THEN
    RETURN 'إجابة صحيحة بالمعنى لهذا السؤال.';
  END IF;
  IF COALESCE(_score, 0) > 0 THEN
    RETURN 'إجابة جزئية لهذا السؤال وتم احتساب الدرجة حسب عناصر الإجابة الصحيحة.';
  END IF;
...
WITH text_answers AS (
  ...
  SELECT
    ta.*,
    COALESCE(ta.alignment->>'model_answer', ta.original_correct_answer, '') AS effective_model_answer,
    COALESCE(ta.alignment->>'source', 'current_model_answer') AS alignment_source,
...
SELECT
  'repair.text_grading_adjacent_model_alignment',
  ua.student_id,
  ua.exam_id,
  ua.id,
  jsonb_build_object(
    'repaired_count', audit.repaired_count,
    'mapping_strategy', 'exam_answers.question_id -> exam_questions.id plus adjacent model-answer safety alignment',
    'uses_question_id', true,
    'uses_array_index', false
  )
FROM audit
...
REVOKE ALL ON FUNCTION public.exam_text_feedback(text, text, numeric, numeric, text) FROM PUBLIC, anon;
