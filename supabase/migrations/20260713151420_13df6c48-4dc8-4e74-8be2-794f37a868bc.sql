...
BEGIN
  ...
  IF NOT FOUND OR v_exam.is_published IS DISTINCT FROM true OR v_exam.status <> 'published'::exam_status THEN
    RETURN jsonb_build_object('success', false, 'error', 'التدريب غير متاح');
  END IF;

  IF NOT public.is_modrek_training_exam_accessible(_exam_id, v_student) THEN
    RETURN jsonb_build_object('success', false, 'error', 'هذا التدريب تابع لطالب آخر أو غير متاح');
  END IF;
  ...
  IF _attempt_id IS NOT NULL THEN
    ...
    IF FOUND THEN
      IF v_attempt.status = 'in_progress'::exam_attempt_status THEN
        RETURN jsonb_build_object('success', true, 'attempt_id', v_attempt.id, 'resumed', true, 'training_exam', true);
      END IF;
      RETURN jsonb_build_object('success', false, 'error', 'تم تسليم هذه المحاولة مسبقاً', 'attempt_id', v_attempt.id, 'training_exam', true);
    END IF;
  END IF;
  ...
  IF v_in_progress IS NOT NULL THEN
    RETURN jsonb_build_object('success', true, 'attempt_id', v_in_progress, 'resumed', true, 'training_exam', true);
  END IF;
  ...
  IF v_existing_count >= COALESCE(v_exam.max_attempts, 999) THEN
    RETURN jsonb_build_object('success', false, 'error', 'تم استنفاد عدد المحاولات', 'training_exam', true);
  END IF;
  ...
  IF COALESCE(v_max_score, 0) <= 0 THEN
    RETURN jsonb_build_object('success', false, 'error', 'لا توجد أسئلة صالحة في هذا التدريب', 'training_exam', true);
  END IF;
  ...
  RETURN jsonb_build_object('success', true, 'attempt_id', v_attempt_id, 'resumed', false, 'training_exam', true);
END;
$function$;
...
BEGIN
  ...
  IF COALESCE(v_exam.source, 'teacher') = 'modrek_ai'
    ...
    RETURN jsonb_build_object('success', false, 'error', 'هذا التدريب تابع لطالب آخر أو غير متاح', 'training_exam', true);
  END IF;
  ...
  IF v_in_progress IS NOT NULL THEN
    RETURN jsonb_build_object('success', true, 'attempt_id', v_in_progress, 'resumed', true, 'training_exam', false);
  END IF;
  ...
  EXCEPTION WHEN unique_violation THEN
    ...
    RETURN jsonb_build_object('success', true, 'attempt_id', v_attempt_id, 'resumed', true, 'training_exam', false);
  END;
  ...
  RETURN jsonb_build_object('success', true, 'attempt_id', v_attempt_id, 'resumed', false, 'training_exam', false);
END;
$function$;
...
CREATE OR REPLACE FUNCTION public.get_modrek_training_questions_for_attempt(_attempt_id uuid)
RETURNS jsonb
LANGUAGE plpgsql
STABLE SECURITY DEFINER
SET search_path TO 'public'
...
BEGIN
  ...
  IF v_exam_id IS NULL OR NOT public.is_modrek_training_exam_accessible(v_exam_id, v_uid) THEN
    RAISE EXCEPTION 'not authorized';
  END IF;

  RETURN public.get_exam_questions_for_student(v_exam_id);
END;
$function$;
  ...
  USING (
    EXISTS (
      ...
      WHERE a.exam_id = exams.id
        ...
        AND (
          COALESCE(exams.source, 'teacher') = 'modrek_ai'
          OR exams.owner_student_id = auth.uid()
          OR (exams.teacher_id IS NULL AND exams.group_id IS NULL)
        )
    )
  );
END $$;
