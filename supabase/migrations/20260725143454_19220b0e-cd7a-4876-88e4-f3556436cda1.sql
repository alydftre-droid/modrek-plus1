...
AS $function$
  WITH normalized_target AS (
    ...
    SELECT
      CASE
        -- Older exam creation code copied the current group education type into
        -- target_education_type even when the teacher chose “الجميع”. For all-section
        -- broadcast exams this must behave as all education types, otherwise أدبي/أزهر
        -- sibling groups are hidden incorrectly.
        WHEN nt.section IS NULL
...
BEGIN
  ...
  IF NOT (v_is_admin OR v_has_group_access) THEN
    RETURN QUERY SELECT
      ...
      'الطالب غير مشترك في هذه المجموعة. لو المحتوى يظهر كمقفول فهذا طبيعي، لكن الامتحان لن يفتح إلا بعد الاشتراك أو تفعيله كمجاني إن كان ذلك مطلوباً.'::text,
      'src/components/exams/StudentExamPanel.tsx + database:student_group_purchases'::text,
      'debug_student_group_exam_visibility'::text,
  ...
  SELECT
    ...
    CASE
      ...
      WHEN NOT c.c_in_broadcast_scope THEN 'الامتحان ليس داخل المجموعة الحالية ولا داخل مجموعات البث الشقيقة. السبب غالباً اختلاف subject/shared_subject أو teacher_id/created_by بين مجموعات علمي وأدبي.'
      ...
      WHEN NOT (v_is_admin OR c.c_target_matches)
        ...
        AND public.normalize_content_education_type(c.target_education_type) IS NOT NULL
        AND public.normalize_content_education_type(c.target_education_type) = c.c_exam_group_edu
      THEN 'السبب الحقيقي: كود إنشاء الامتحان نسخ نوع تعليم المجموعة داخل target_education_type رغم أن المعلم اختار الجميع، فحجب الامتحان عن طلاب أدبي/أزهر في المجموعة الشقيقة. الملف: src/hooks/useExamMutations.ts.'
      WHEN NOT (v_is_admin OR c.c_target_matches)
        AND public.normalize_content_education_type(c.target_education_type) IS NOT NULL
        AND v_student_education_type IS DISTINCT FROM public.normalize_content_education_type(c.target_education_type)
      ...
      ELSE 'الامتحان ظاهر حسب قواعد الكتالوج الحالية. إذا لم يظهر في الواجهة فالسبب في فلترة StudentExamPanel أو useStudentExamCatalog.'
    END AS reason,
    CASE
...
SET search_path TO 'public'
AS $function$
  SELECT * FROM public.debug_student_group_exam_visibility(_group_id, _sub_subject_id)
$function$;
...
GRANT EXECUTE ON FUNCTION public.debug_student_group_exam_visibility(uuid, uuid) TO anon;
