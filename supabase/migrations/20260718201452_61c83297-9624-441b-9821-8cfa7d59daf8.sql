...
BEGIN
  ...
  IF NEW.group_id IS NOT NULL THEN
    ...
    -- Some older monthly groups have no purchase rows, but students are linked
    -- to the teacher through choices. Keep notifications alive for that legacy path.
    IF COALESCE(array_length(candidate_ids, 1), 0) = 0 THEN
      ...
      WHERE stc.teacher_id = NEW.uploaded_by
        AND stc.category = subject_rec.category
