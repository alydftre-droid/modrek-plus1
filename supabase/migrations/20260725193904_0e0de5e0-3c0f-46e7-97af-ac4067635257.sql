-- Restore strict student group isolation after broad broadcast changes.
-- Root cause: previous exam broadcast logic expanded catalog results across sibling groups,
-- and student catalog admin bypass let developer/test accounts skip section targeting.
...
BEGIN
  ...
  WITH candidate_content AS (
    ...
    WHERE c.group_id = _group_id
      ...
      -- IMPORTANT: no admin bypass here. This is the student catalog, so section/education
      -- targeting must always reflect the active student profile to avoid mixed علمي/أدبي rows.
...
BEGIN
  ...
  WITH candidate_exams AS (
    ...
    WHERE e.group_id = _group_id
      ...
      -- IMPORTANT: no admin bypass here. A student-facing group tab must not mix exams
      -- from another target section/education type.
