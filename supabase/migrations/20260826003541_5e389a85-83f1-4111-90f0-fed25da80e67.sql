...
BEGIN
  ...
  SET status = 'retrying',
      error = COALESCE(j.error, 'انقطع نبض العامل أثناء عملية PDF طويلة أو تعطل تنفيذ المرحلة؛ تمت إعادة الجدولة تلقائياً. راجع آخر نبض/السجل لمعرفة آخر خطوة.'),
      output = COALESCE(j.output, '{}'::jsonb) || jsonb_build_object(
        'stale_recovery', jsonb_build_object(
          'reason', 'worker heartbeat stale',
          'stale_after_minutes', 6,
          'recovered_at', now(),
          'hint', 'إذا تكررت الرسالة فالمشكلة غالباً في قراءة/تقسيم PDF أو حجم الصفحة وليس في الرفع نفسه.'
        )
      ),
      finished_at = now(),
      next_run_at = now() + interval '45 seconds',
      updated_at = now()
  WHERE j.status = 'running'
    AND COALESCE(j.updated_at, j.started_at, j.created_at) < now() - interval '6 minutes';
  ...
  SET pipeline_stage = 'failed',
      error_message = COALESCE((
        ...
        WHERE j.version_id = v.id
          AND j.status = 'failed'
          AND j.kind NOT IN ('extract_page'::public.processing_job_kind, 'split_pdf'::public.processing_job_kind)
  ...
  WHERE j.id = (
    ...
    WHERE candidate.status IN ('pending','retrying')
      ...
      AND (
        candidate.kind <> 'merge_text'::public.processing_job_kind
        OR NOT EXISTS (
          SELECT 1
          FROM public.processing_jobs extracting
          WHERE extracting.version_id = candidate.version_id
            AND extracting.kind IN ('split_pdf'::public.processing_job_kind, 'extract_text'::public.processing_job_kind, 'extract_page'::public.processing_job_kind)
            AND extracting.status IN ('pending','running','retrying')
            AND extracting.attempts < COALESCE(extracting.max_attempts, 3)
        )
      )
      AND (
        ...
        OR NOT EXISTS (
          ...
          WHERE running_page.kind = 'extract_page'::public.processing_job_kind
            ...
            AND COALESCE(running_page.updated_at, running_page.started_at, running_page.created_at) > now() - interval '6 minutes'
        )
      )
      AND (
        candidate.kind <> 'extract_page'::public.processing_job_kind
...
SET status = 'pending',
    ...
    output = COALESCE(output, '{}'::jsonb) || jsonb_build_object(
      'manual_recovery', jsonb_build_object(
        'reason', 'modrek pdf worker heartbeat/claim fix applied',
        'recovered_at', now()
      )
    ),
    updated_at = now()
WHERE status IN ('retrying','failed')
  AND kind IN ('extract_text'::public.processing_job_kind, 'split_pdf'::public.processing_job_kind, 'extract_page'::public.processing_job_kind, 'merge_text'::public.processing_job_kind)
  AND (
    COALESCE(error, '') ILIKE '%انقطع نبض%'
    OR COALESCE(error, '') ILIKE '%مهلة المرحلة%'
    OR COALESCE(error, '') ILIKE '%worker heartbeat%'
  );
...
WHERE EXISTS (
  ...
  WHERE j.version_id = v.id
    ...
    AND j.kind IN ('extract_text'::public.processing_job_kind, 'split_pdf'::public.processing_job_kind, 'extract_page'::public.processing_job_kind, 'merge_text'::public.processing_job_kind)
);
...
WHERE EXISTS (
  ...
  JOIN public.processing_jobs j ON j.version_id = v.id
  WHERE v.source_id = s.id
    AND v.is_current = true
    AND j.status IN ('pending','retrying')
    AND j.kind IN ('extract_text'::public.processing_job_kind, 'split_pdf'::public.processing_job_kind, 'extract_page'::public.processing_job_kind, 'merge_text'::public.processing_job_kind)
