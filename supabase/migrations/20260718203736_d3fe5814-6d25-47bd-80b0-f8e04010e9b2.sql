...
BEGIN
  ...
  target_section := CASE
    WHEN lower(trim(COALESCE(group_rec.section_name, subject_rec.section, ''))) IN ('scientific', 'علمي', 'علمي علوم', 'علمي رياضة') THEN 'scientific'
  ...
  scope_link := CASE WHEN NEW.type = 'exam' THEN '/student/exams' ELSE regexp_replace(target_link, '&content_id=[^&]*', '') END;
  ...
  IF NEW.group_id IS NOT NULL THEN
    ...
    IF COALESCE(array_length(candidate_ids, 1), 0) = 0 THEN
      SELECT COALESCE(array_agg(DISTINCT stc.student_id), '{}'::uuid[])
      INTO candidate_ids
  ...
  IF COALESCE(array_length(candidate_ids, 1), 0) = 0 THEN
    RAISE LOG 'content notification skipped: no candidates content_id=% teacher=% subject=% group=% category=% variants=%', NEW.id, NEW.uploaded_by, NEW.subject_id, NEW.group_id, subject_rec.category, category_variants;
    RETURN NEW;
  END IF;
  ...
  WITH eligible AS (
    ...
    JOIN eligible e ON e.id = n.user_id
    WHERE n.created_by = NEW.uploaded_by
    ...
    JOIN public.notifications n
     ...
     AND COALESCE(n.link, '') <> target_link
    WHERE NOT EXISTS (SELECT 1 FROM exact_existing x WHERE x.user_id = e.id)
  ), updated AS (
  ...
  WHERE NOT EXISTS (SELECT 1 FROM updated u WHERE u.user_id = e.id)
    AND NOT EXISTS (SELECT 1 FROM exact_existing x WHERE x.user_id = e.id);
...
REVOKE EXECUTE ON FUNCTION public.trg_content_automation() FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.trg_content_automation() TO authenticated;
