...
AS $function$
  WITH normalized AS (
    SELECT
      public.resolve_shared_subject_key(p_target_category, p_target_name) AS target_key,
      public.resolve_shared_subject_key(p_candidate_category, p_candidate_name) AS candidate_key,
      public.normalize_price_scope_text(p_target_category) AS target_category_norm,
      public.normalize_price_scope_text(p_candidate_category) AS candidate_category_norm,
      public.normalize_price_scope_text(p_target_name) AS target_name_norm,
      public.normalize_price_scope_text(p_candidate_name) AS candidate_name_norm
  )
  SELECT CASE
    WHEN target_key IS NOT NULL AND candidate_key IS NOT NULL THEN target_key = candidate_key
    WHEN target_key IS NULL AND candidate_key IS NULL THEN (
      target_category_norm = candidate_category_norm
      AND target_name_norm = candidate_name_norm
    )
    ELSE false
  END
  FROM normalized
...
REVOKE EXECUTE ON FUNCTION public.catalog_subjects_match(text, text, text, text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.catalog_subjects_match(text, text, text, text) TO authenticated, service_role, supabase_read_only_user;
...
BEGIN
  ...
  WITH target_subject AS (
    ...
    SELECT
      ...
      COALESCE(target_match.id, c.sub_subject_id) AS c_sub_subject_id,
...
BEGIN
  ...
  WITH target_subject AS (
    ...
    SELECT
      e.*,
      COALESCE(target_match.id, e.sub_subject_id) AS matched_sub_subject_id,
...
GRANT EXECUTE ON FUNCTION public.get_student_group_content_catalog(uuid, uuid) TO authenticated, service_role, supabase_read_only_user;
