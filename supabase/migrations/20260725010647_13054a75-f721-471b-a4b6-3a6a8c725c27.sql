...
-- Strict group isolation: student catalog RPCs must only return content
-- belonging to the exact group the student is viewing.
...
AS $function$
BEGIN
  -- Group isolation is now strict; the primary catalog handles all students.
  RETURN QUERY
  SELECT * FROM public.get_student_group_content_catalog(_group_id, NULL);
END;
$function$;
