...
BEGIN
  ...
  SELECT COALESCE(array_agg(DISTINCT ls.id), ARRAY[]::uuid[])
  INTO v_session_ids
  FROM public.live_sessions ls
  WHERE ls.teacher_id = _teacher_id
    AND ls.group_id = ANY(v_group_ids);
  ...
  WHERE id = ANY(v_exam_ids);

  DELETE FROM public.content
  WHERE id = ANY(v_content_ids);

  DELETE FROM public.ai_lessons
  WHERE created_by = _teacher_id
    AND (group_id = ANY(v_group_ids) OR (group_id IS NULL AND subject_id = ANY(v_subject_ids)));
  ...
  WHERE stc.teacher_id = _teacher_id
    AND stc.stage = a.stage
    AND stc.grade = a.grade
    AND stc.category = a.category;
  ...
  RETURN jsonb_build_object(
    ...
    'deleted_content', cardinality(v_content_ids),
    'deleted_exams', cardinality(v_exam_ids),
    'deleted_live_sessions', cardinality(v_session_ids)
  );
END;
$$;
