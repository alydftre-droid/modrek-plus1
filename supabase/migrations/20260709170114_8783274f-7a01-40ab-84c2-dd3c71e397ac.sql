...
AS $$
  SELECT CASE trim(COALESCE(_value, ''))
    WHEN 'general' THEN 'عام'
    WHEN 'عام' THEN 'عام'
    WHEN 'azhar' THEN 'أزهر'
    WHEN 'أزهر' THEN 'أزهر'
    ELSE NULL
  END
$$;
...
AS $$
  SELECT CASE trim(COALESCE(_value, ''))
    ...
    WHEN 'علمي' THEN 'scientific'
    WHEN 'علمي علوم' THEN 'scientific'
    WHEN 'علمي رياضة' THEN 'scientific'
    WHEN 'literary' THEN 'literary'
    WHEN 'أدبي' THEN 'literary'
    ELSE NULL
  END
$$;
  ...
  SELECT EXISTS (
    ...
    WHERE p.id = _student_id
      AND (
        ...
        OR public.normalize_exam_target_education_type(p.education_type) IS NULL
      ...
      AND (
        ...
        OR public.normalize_exam_target_section(p.section) IS NULL
        OR public.normalize_exam_target_section(p.section) = public.normalize_exam_target_section(_target_section)
      )
  )
$$;

REVOKE EXECUTE ON FUNCTION public.exam_target_matches_student(uuid, text, text) FROM PUBLIC, anon;
