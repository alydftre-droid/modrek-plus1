-- Ensure PostgREST schema cache recognizes the free-preview admin RPC.
-- This migration is intentionally non-destructive: it does not update any content rows.
...
DECLARE
  ...
  v_actor_email text := lower(COALESCE(v_claims->>'email', ''));
...
BEGIN
  IF COALESCE(v_claims->>'role', '') <> 'service_role'
     AND v_actor_email <> 'alyedaft@gmail.com'
...
COMMENT ON FUNCTION public.admin_set_content_free_preview(uuid, boolean)
IS 'Admin-only RPC used by Mudrik Plus developer tools to toggle free-preview status across related content rows.';
