  ...
  WHERE j.status = 'retrying'
    ...
    AND NOT EXISTS (
      ...
      WHERE e.job_id = j.id
        ...
        AND (e.data->>'category') = 'rate_limit'
    );
...
CREATE INDEX IF NOT EXISTS idx_processing_jobs_extract_running
  ON public.processing_jobs(kind, status, updated_at DESC)
  WHERE kind = 'extract_page'::public.processing_job_kind;

UPDATE public.processing_jobs
SET max_attempts = GREATEST(COALESCE(max_attempts, 0), 20),
    next_run_at = CASE
      WHEN status = 'retrying' AND lower(COALESCE(error, '')) LIKE ANY (ARRAY['%quota%', '%rate-limit%', '%rate limit%', '%429%', '%resource exhausted%'])
