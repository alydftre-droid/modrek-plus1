...
BEGIN
  ...
  -- New students this period (first time they appeared for this teacher)
  ...
  -- Withdrawal open day (from platform settings if available)
  BEGIN
    SELECT COALESCE(
  ...
  -- Per-grade focused-group breakdown (used by GradeEarningsTable on replay)
  SELECT COALESCE(jsonb_object_agg(gd.key, gd.groups), '{}'::jsonb)
  ...
  FROM (
    SELECT
      ...
      jsonb_agg(
        jsonb_build_object(
          'id', ter.group_id,
          'title', COALESCE(cg.title, 'مجموعة'),
          'price', COALESCE(cg.price, MAX(ter.gross_amount)),
          'count', COUNT(DISTINCT ter.student_id),
          'net', SUM(ter.net_amount)
        )
  ...
  LATERAL (
    SELECT gd_rows.key AS key, jsonb_agg(row_to_json(gd_rows)) AS groups
  ) gd;

  -- Rebuild per-grade groups aggregation properly
  WITH per_group AS (
    SELECT
      concat_ws('__', COALESCE(s.stage,''), COALESCE(s.grade,''), COALESCE(s.category,'')) AS key,
      ter.group_id AS id,
  ...
  -- Growth series: last 5 archived months + this one (for the LineChart)
  WITH last5 AS (
  ...
  -- Grade history: last 5 archives per key, plus current values
  WITH last5 AS (
  ...
  -- Move frozen -> available; also reset frozen to 0
  UPDATE public.teacher_wallets
