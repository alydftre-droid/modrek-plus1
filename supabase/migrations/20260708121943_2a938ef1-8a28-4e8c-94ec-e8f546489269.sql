...
DECLARE
  ...
  v_publishable_key text := 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InFvaGhybGlhZWNkdGFleWZoY3ZiIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NjU3MTU1NDYsImV4cCI6MjA4MTI5MTU0Nn0.0j-tjPRX-s2wMCYfJypWo2dlYk9Mi40ueU8z0f00y8A';
  v_request_id bigint;
BEGIN
  ...
  INSERT INTO public.notification_delivery_logs (
    ...
    p_body,
    COALESCE(p_link, ''),
    jsonb_build_object('request_id', v_request_id, 'function_url', 'send-push-notification', 'target', 'lovable-cloud')
  );
EXCEPTION WHEN OTHERS THEN
  INSERT INTO public.notification_delivery_logs (
    ...
    p_body,
    COALESCE(p_link, ''),
    jsonb_build_object('error', SQLERRM, 'target', 'lovable-cloud')
  );
