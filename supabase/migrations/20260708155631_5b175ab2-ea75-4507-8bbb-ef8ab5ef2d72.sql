...
AS $$
  ...
  teacher_ids AS (
    SELECT teacher_id FROM chosen
    UNION
    SELECT teacher_id FROM paid
  )
  SELECT
    ti.teacher_id,
...
DROP FUNCTION IF EXISTS public.get_developer_student_overview(uuid);

CREATE FUNCTION public.get_developer_student_overview(_student_id uuid)
RETURNS jsonb
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
