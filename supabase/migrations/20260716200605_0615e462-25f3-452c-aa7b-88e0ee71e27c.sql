...
AS $$
  SELECT CASE
    ...
    WHEN lower(btrim(coalesce(_section, ''))) IN ('literary') THEN 'literary'
    WHEN btrim(coalesce(_section, '')) IN ('أدبي','ادبي','أدبى','ادبى') THEN 'literary'
    ELSE lower(btrim(coalesce(_section, '')))
  END
$$;

CREATE OR REPLACE FUNCTION public.library_canonical_track_name(_code text)
RETURNS text
LANGUAGE sql
IMMUTABLE
SET search_path = public
...
CREATE OR REPLACE FUNCTION public.library_canonical_track_sort(_code text)
RETURNS integer
LANGUAGE sql
IMMUTABLE
SET search_path = public
...
CREATE OR REPLACE FUNCTION public.library_ensure_track_code(_code text)
RETURNS uuid
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
AS $$
DECLARE
  _normalized text := nullif(btrim(coalesce(_code, '')), '');
  _id uuid;
BEGIN
  ...
  IF _normalized IN ('sci_science', 'sci_math') THEN
    _normalized := 'scientific';
  END IF;

  INSERT INTO public.library_tracks (code, name_ar, sort_order, is_active)
  VALUES (
    _normalized,
    public.library_canonical_track_name(_normalized),
    public.library_canonical_track_sort(_normalized),
    true
  )
...
SELECT public.library_ensure_track_code('none');
SELECT public.library_ensure_track_code('scientific');
SELECT public.library_ensure_track_code('sci_science');
SELECT public.library_ensure_track_code('sci_math');
SELECT public.library_ensure_track_code('literary');
...
BEGIN
  ...
  IF NEW.track_id IS NULL AND _subject_stage_code = 'secondary' AND coalesce(_subject_track_code, '') <> '' THEN
    NEW.track_id := public.library_ensure_track_code(_subject_track_code);
  END IF;
  ...
  IF NEW.track_id IS NOT NULL THEN
    ...
    IF NOT FOUND OR _track.is_active IS NOT TRUE THEN
      IF _subject_stage_code = 'secondary' AND coalesce(_subject_track_code, '') <> '' THEN
        NEW.track_id := public.library_ensure_track_code(_subject_track_code);
        SELECT id, code, name_ar, is_active INTO _track
        FROM public.library_tracks
        WHERE id = NEW.track_id;
    ...
    IF NOT FOUND OR _track.is_active IS NOT TRUE THEN
      RAISE EXCEPTION USING MESSAGE = 'library_debug.invalid_track_id', DETAIL = jsonb_build_object('file','database:function:validate_library_book_scope_before_write','function','validate_library_book_scope_before_write','component','LibraryUploadPage','hook','React useState/useMemo → callAdmin','api','library-admin?action=create/update','table','library_tracks','column','track_id','sent_value',NEW.track_id,'expected_value','public.library_tracks.id موجود وفعّال أو شعبة مستنتجة من المادة','correct_value',_subject_track_code,'sql_query','SELECT id,code,name_ar,is_active FROM public.library_tracks WHERE id = $1','query_result','no_rows_or_inactive','failure_reason','track_not_found_or_inactive','layer','database','error_type','validation_error')::text;
    END IF;
...
REVOKE ALL ON FUNCTION public.library_canonical_track_name(text) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.library_canonical_track_sort(text) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.library_ensure_track_code(text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.library_track_code_from_subject(text) TO anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.library_canonical_track_name(text) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.library_canonical_track_sort(text) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.library_ensure_track_code(text) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.validate_library_book_scope_before_write() TO authenticated, service_role;
