...
DECLARE
  ...
  normalized_group_section TEXT;
  normalized_subject_section TEXT;
  target_section TEXT;
  target_link TEXT;
...
BEGIN
  ...
  target_education := NULLIF(COALESCE(NEW.education_type, v_group_education_type, ''), '');
  target_education := CASE
    WHEN lower(trim(target_education)) IN ('both', 'all', 'none', 'null', 'الكل', 'كلاهما') THEN NULL
    ELSE target_education
  END;

  normalized_group_section := lower(trim(COALESCE(v_group_section_name, '')));
  normalized_subject_section := lower(trim(COALESCE(subject_rec.section, '')));

  target_section := CASE
    WHEN normalized_group_section IN ('scientific', 'علمي', 'علمي علوم', 'علمي رياضة', 'علم') THEN 'scientific'
    WHEN normalized_group_section IN ('literary', 'أدبي', 'ادبي') THEN 'literary'
    WHEN normalized_subject_section IN ('scientific', 'علمي', 'علمي علوم', 'علمي رياضة', 'علم') THEN 'scientific'
    WHEN normalized_subject_section IN ('literary', 'أدبي', 'ادبي') THEN 'literary'
    ELSE NULL
  END;
