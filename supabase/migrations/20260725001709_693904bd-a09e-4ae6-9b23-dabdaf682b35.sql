...
BEGIN
  ...
  WITH target_subject AS (
    ...
    SELECT
      ...
      CASE
        WHEN c.group_id = _group_id THEN 0
        WHEN public.content_effective_section(c.target_section, c.subject_id, c.group_id) IS NULL THEN 1
...
GRANT EXECUTE ON FUNCTION public.get_student_group_content_diagnostics(uuid, uuid) TO authenticated;
...
GRANT EXECUTE ON FUNCTION public.content_target_matches_student(text, uuid, uuid, uuid, text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.content_effective_section(text, uuid, uuid) TO authenticated;
