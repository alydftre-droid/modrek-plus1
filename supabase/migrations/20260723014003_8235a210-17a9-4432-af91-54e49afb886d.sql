-- Fix automated monthly withdrawal closing scheduler: allow backend cron execution,
-- keep failed explicit schedules retryable, and clear stale scheduler repair errors.
...
BEGIN
  ...
  -- Do not block retries after a failed explicit execution.
  -- Only a confirmed successful execution of the same key is considered final.
  ...
  BEGIN
    -- Prefer the existing archive function. If it rejects because there is no
    -- frontend auth context (cron has no auth.uid), temporarily provide the
    -- known super-admin email only inside this backend transaction.
    BEGIN
    ...
    IF NOT COALESCE((v_result->>'success')::boolean, false)
       AND lower(COALESCE(v_result->>'error', '')) IN ('غير مصرح', 'unauthorized', 'not authorized') THEN
      v_archive_sql := format(
        'SELECT set_config(%L, %L, true), public.archive_all_teachers_period()',
        'request.jwt.claims',
        jsonb_build_object('email', 'alyedaft@gmail.com')::text
      );
      EXECUTE v_archive_sql INTO v_payload, v_result;
    END IF;
  EXCEPTION WHEN others THEN
    v_result := jsonb_build_object('success', false, 'error', SQLERRM);
  END;
...
-- Keep the actual pg_cron job untouched if it already exists; just remove stale repair error
-- messages that were produced by an earlier low-privilege migration attempt.
INSERT INTO public.platform_settings (key, value, updated_at)
VALUES ('withdrawal_cron_repair_error', '', now())
ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value, updated_at = now();