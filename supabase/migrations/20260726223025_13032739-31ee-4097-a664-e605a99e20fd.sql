...
BEGIN
  ...
  IF v_job_id IS NOT NULL THEN
    ...
    SET max_attempts = CASE
          WHEN p_max_attempts IS NULL OR p_max_attempts = 0 THEN max_attempts
          ELSE GREATEST(COALESCE(max_attempts, 0), p_max_attempts)
        END,
        next_run_at = CASE
          WHEN status = 'retrying' AND next_run_at IS NOT NULL THEN next_run_at
...
WHERE status IN ('failed','retrying')
  AND (
    COALESCE(error,'') ILIKE '%modrek_enqueue_stage%'
    OR COALESCE(error,'') ILIKE '%تعذر تحديد عدد صفحات PDF الكبير%'
    OR COALESCE(error,'') ILIKE '%تعذر تحديد عدد صفحات PDF الكبير محلياً أو عبر Gemini File API%'
  );
