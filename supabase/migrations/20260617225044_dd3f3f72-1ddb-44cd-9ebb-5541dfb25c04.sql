CREATE OR REPLACE FUNCTION public.sync_exam_attempts_count(_exam_id uuid)
RETURNS void
LANGUAGE sql
SECURITY DEFINER
...
AS $$
  ...
  SET total_attempts_count = COALESCE((
    SELECT COUNT(*)::int
    FROM public.exam_attempts a
    WHERE a.exam_id = _exam_id
      AND a.status IN ('submitted'::exam_attempt_status, 'graded'::exam_attempt_status, 'expired'::exam_attempt_status)
  ), 0)
  WHERE e.id = _exam_id;
$$;

GRANT EXECUTE ON FUNCTION public.sync_exam_attempts_count(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.sync_exam_attempts_count(uuid) TO service_role;
REVOKE EXECUTE ON FUNCTION public.sync_exam_attempts_count(uuid) FROM anon;

UPDATE public.exams e
SET total_attempts_count = COALESCE(c.finished_count, 0)
FROM (
  SELECT e2.id, COUNT(a.id)::int AS finished_count
...
GRANT EXECUTE ON FUNCTION public.submit_exam_attempt(uuid, integer, integer) TO service_role;
