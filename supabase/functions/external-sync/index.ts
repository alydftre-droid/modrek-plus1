...
-- content: only the current term workspace is readable/manageable
...
CREATE POLICY "Teachers can view own current-term content" ON public.content
  FOR SELECT TO authenticated
  USING (
...
CREATE POLICY "Teachers can insert current-term content" ON public.content
  FOR INSERT TO authenticated
  WITH CHECK (
    auth.uid() = uploaded_by
    AND COALESCE(type, '') <> 'student_library'
...
CREATE POLICY "Teachers can update own current-term content" ON public.content
  FOR UPDATE TO authenticated
  USING (
    auth.uid() = uploaded_by
    AND COALESCE(type, '') <> 'student_library'
...
CREATE POLICY "Public can view accessible current-term content" ON public.content
  FOR SELECT TO public
  USING (
...
-- exams: only the current term workspace is readable/manageable
...
CREATE POLICY "Teachers can view own current-term exams" ON public.exams
  FOR SELECT TO authenticated
  USING (
    auth.uid() = teacher_id
    AND public.term_item_matches_current_system_term(subject_id, group_id, term)
...
CREATE POLICY "Teachers can create own current-term exams" ON public.exams
  FOR INSERT TO authenticated
  WITH CHECK (
    auth.uid() = teacher_id
    AND public.term_item_matches_current_system_term(subject_id, group_id, term)
...
CREATE POLICY "Teachers can update own current-term exams" ON public.exams
  FOR UPDATE TO authenticated
  USING (
    auth.uid() = teacher_id
    AND public.term_item_matches_current_system_term(subject_id, group_id, term)
...
CREATE POLICY "Teachers can delete own current-term exams" ON public.exams
  FOR DELETE TO authenticated
  USING (
    auth.uid() = teacher_id
    AND public.term_item_matches_current_system_term(subject_id, group_id, term)
